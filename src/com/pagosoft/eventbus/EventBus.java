 ...
 * (the editor component needs to startup a new editor for it, the
...
public class EventBus {
	 ...
	 *
	 * If the event is vetoable and one of the listeners veto against the
	 * event, this method will return false. It will return true otherwise.
	 *
	 * @param event
	 */
	public boolean fireEvent(ApplicationEvent event) {
		...
		for(int i = length-1; i >= 0; i--) {
			...
			if(event instanceof VetoableEvent && ((VetoableEvent)event).isVetoed()) {
				return false;
			}
		}
		return true;
